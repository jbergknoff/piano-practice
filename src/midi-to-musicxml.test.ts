...
function withDeltas(pairs: Array<[number, Record<string, unknown>]>): MidiEvent[] {
...
describe("midiToMusicXml – programmatic fixtures", () => {
	...
	test("C major scale produces two measures of four quarter notes each", () => {
		...
		const pairs: Array<[number, Record<string, unknown>]> = [
			...
			[0, { type: "timeSignature", meta: true, numerator: 4, denominator: 4, metronome: 24, thirtyseconds: 8 }],
			[0, { type: "keySignature", meta: true, key: 0, scale: 0 }],
		];
		for (let i = 0; i < noteNumbers.length; i++) {
			const tick = i * TPB;
			pairs.push([tick, { type: "noteOn", channel: 0, noteNumber: noteNumbers[i], velocity: 64 }]);
			pairs.push([tick + TPB, { type: "noteOff", channel: 0, noteNumber: noteNumbers[i], velocity: 0 }]);
		}
	...
	test("simultaneous notes are grouped as a chord", () => {
		...
		for (const n of chord) {
			pairs.push([0, { type: "noteOn", channel: 0, noteNumber: n, velocity: 64 }]);
		}
		for (const n of chord) {
			pairs.push([TPB, { type: "noteOff", channel: 0, noteNumber: n, velocity: 0 }]);
		}
	...
	test("3/4 time signature is preserved and measures have three beats", () => {
		...
		const pairs: Array<[number, Record<string, unknown>]> = [
			[0, { type: "timeSignature", meta: true, numerator: 3, denominator: 4, metronome: 24, thirtyseconds: 8 }],
		];
		for (let i = 0; i < noteNumbers.length; i++) {
			const tick = i * TPB;
			pairs.push([tick, { type: "noteOn", channel: 0, noteNumber: noteNumbers[i], velocity: 64 }]);
...
describe("midiToMusicXml – music21-test06.mid fixture", () => {
	...
	test("produces multiple measures", () => {
		...
		const measureNums = [...xml.matchAll(/number="(\d+)"/g)].map((m) => Number(m[1]));
