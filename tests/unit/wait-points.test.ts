...
describe("buildWaitPoints – slashed grace notes", () => {
  ...
  test("slashed grace whose beat collides with a required note from an earlier event is optional at both that wait point and the next", () => {
    ...
    // D5 must be optional at BOTH the E5 wait point (the user may play it
    // there without penalty) and the C5 wait point (which it actually ornaments).
...
<score-partwise version="3.1">
  ...
  <part id="P1">
    ...
    expect(e5Point?.optionalNoteNumbers.has(74)).toBe(true); // D5 = 74 optional here too
    expect(e5Point?.noteNumbers.has(74)).toBe(false); // D5 not required
