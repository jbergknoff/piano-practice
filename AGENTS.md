...
| `src/selection.ts` | `MeasureRange` type plus `selectionStartBeat`/`selectionEndBeat`/`measureInRange`/`selectionKeyForRange`/`selectionLabelForRange` — the range→beat conversion, the per-note measure-membership test, the persisted selection key, and the human-readable label, each previously computed independently in the three mode hooks, `PracticeScreen`, and the custom-ranges UI |
...
| `src/hooks/use-file-history.ts` | localStorage persistence built on two primitives (`loadJson`/`saveJson`) and a generic `createAttemptStore<T>` factory shared by the wait-mode and playalong attempt logs (same load/save/delete/clear shape, different storage prefix and record type): per-file history (BPM, range, mode, cursor) + attempt log + `computeLibrarySummary` (aggregates last-practiced date/mode/best-score across every selection key and both attempt kinds, for the library list) |
...
(`MeasureJumpModal`, `RangeNameModal`, `BpmInputModal`) now use
