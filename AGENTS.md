...
| `src/selection.ts` | `MeasureRange` type plus `selectionStartBeat`/`selectionEndBeat`/`selectionKeyForRange`/`selectionLabelForRange` — the range→beat conversion, the persisted selection key, and the human-readable label, each previously computed independently in the three mode hooks, `PracticeScreen`, and the custom-ranges UI |
