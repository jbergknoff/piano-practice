...
- `src/hooks/` — `use-piano.ts` (unified piano-connection controller), `use-bluetooth.ts` + `use-web-midi.ts` (the two input transports), `piano-preference.ts` (the remembered transport), `use-wake-lock.ts`, `use-fullscreen.ts`, `use-file-history.ts`, `use-file-library.ts`, `use-bookmarks.ts`
...
| `src/modes/mode-control.ts` | `ModeControl` / `ModeHandle` interfaces, the `PracticeMode` type (`"wait" \| "playalong" \| "listen"`), and `createPlayerHandle` (stable handle that delegates to whatever `MidiPlayer` the getter currently returns) |
...
| `src/selection.ts` | `MeasureRange` type plus `selectionStartBeat`/`selectionEndBeat`/`measureInRange`/`selectionKeyForRange`/`selectionLabelForRange` — the range→beat conversion, the per-note measure-membership test, the persisted selection key, and the human-readable label, each previously computed independently in the three mode hooks, `PracticeScreen`, and the bookmarks UI |
| `lib/midi/midi-player.ts` | Class: Web Audio / MIDI playback, seek, BPM, focus-range stop-at-end, count-in scheduling (app-internal; imports `PlaybackNote` from `lib/musicxml/musicxml-playback`) |
| `lib/midi/ble-midi.ts` | BLE MIDI packet parse/build (`parseBLEMIDI`/`buildBLEMIDINote`/`buildBLEMIDIBatch`); `parseBLEMIDI` returns the same minimal `{ note, kind }` shape as `web-midi.ts`'s `parseMidiMessage` — no consumer needs more |
| `lib/musicxml/musicxml-playback.ts` | Derives the `ScoreConversion` (playback notes, timing) from a MusicXML string — shared by the MIDI and direct-load paths; owns the `ScoreConversion`/`PlaybackNote` types and `playbackNoteId` (the `p{partIndex}-m{measureNumber}-n{noteIndex}-v{voiceIndex}` ID contract with the renderer). Imports `parseScore`/types from `@jbergknoff/sheet-music-display` |
...
| `src/hooks/use-file-history.ts` | localStorage persistence built on two primitives (`loadJson`/`saveJson`) and a generic `createAttemptStore<T>` factory shared by the wait-mode and playalong attempt logs (same load/save/delete/clear shape, different storage prefix and record type): per-file history (BPM, range, mode, cursor) + attempt log + the `Bookmark` record (`loadBookmarks`/`saveBookmarks`) + `computeLibrarySummary` (aggregates last-practiced date/mode/best-score across every selection key and both attempt kinds, for the library list) |
| `src/hooks/use-bookmarks.ts` | Owns the per-file **bookmarks** (user-named measure ranges) and the create/rename modal state (`BookmarkEditorState`). Rendered by `BookmarkModal.tsx` and listed in the "Bookmarks" section of `SelectionRangesDrawer.tsx`; reached from the sheet-music context menu ("Bookmark this range" / "Rename …") and the drawer's + button. Storage lives under `piano-practice:bookmarks:`; `loadBookmarks` transparently migrates a hash's list off the pre-rename `piano-practice:custom-ranges:` key on first read (only when the new key is *absent*, so an empty list can't be re-seeded from the stale copy). The migration is marked `TODO(after August 2026)` for removal — see `use-file-history.ts` |
| `src/hooks/use-file-library.ts` | IndexedDB-backed file library: stores every opened file's bytes keyed by the same SHA-256 content hash used by `use-file-history.ts` (`putLibraryEntry`/`getLibraryEntry`/`getAllLibraryEntries`/`getMostRecentlyOpenedEntry`/`deleteLibraryEntry`) plus a one-shot `migrateRecentFileToLibrary` that moves the old single-slot `piano-practice:recent-file` localStorage blob into the library on first run. `deleteLibraryEntry` only removes the cached bytes/list entry — it deliberately leaves `FileHistory`, attempts, and bookmarks alone, so reopening the same file later (which recreates the entry under the same hash) brings its old practice history back automatically |
| `src/hooks/use-piano.ts` | Unified piano-connection controller: merges the two input transports (`useBluetooth` + `useWebMidi`) into one status/error/device view and exposes `connectBluetooth`/`connectMidi` + `bluetoothSupported`/`midiSupported`, routing `sendNote`/`sendNotesBatch` to whichever transport is connected. App calls this (not `useBluetooth` directly) and passes the result down as the `piano` prop. There is deliberately no auto-select/guessing between transports (an earlier version tried to infer Web MIDI vs Bluetooth from permission state, which was fragile and unpredictable — see git history on this file for the abandoned approach) — the UI always asks the user explicitly, since only the user reliably knows how their piano is connected. `connectBluetooth`/`connectMidi` each resolve to whether the attempt ended up connected. It also owns `suggestedSource` — the transport that last connected successfully (persisted via `piano-preference.ts`), which the badge uses to skip the chooser on the first tap. This is **remembering, not guessing**: it is only ever written after a transport actually connects, and it clears itself (`shortcutSpent`) as soon as an attempt fails, so the very next tap falls back to the chooser. That escape hatch is load-bearing — without it, a user whose last session was Bluetooth could never reach USB, since every tap would re-open the Bluetooth picker. |
...
| `src/hooks/use-web-midi.ts` | Web MIDI input transport (`navigator.requestMIDIAccess`); same surface as `useBluetooth`. Listens on **every** real input port (filtering out the ALSA "MIDI Through" loopback via `isThroughPort`) so it doesn't matter which port is the piano. Sees USB pianos and, on Linux, BLE pianos that BlueZ bridges to ALSA (which Web Bluetooth can't reach once BlueZ's MIDI plugin claims the GATT service). **Auto-reconnects silently on mount** when `navigator.permissions.query({name:"midi"})` reports `"granted"`: unlike Web Bluetooth's per-device grant, the Web MIDI permission is persistent, origin-scoped, and needs no user gesture, so a USB piano reconnects with zero taps. The permission check is the gate that keeps this silent — calling `requestMIDIAccess` unguarded could raise a prompt with no gesture behind it — and a failed auto-attempt falls back to `"idle"` rather than `"error"`, so no modal appears on load |
...
Three modes — the `PracticeMode` type (`"wait" | "playalong" | "listen"`, defined in `mode-control.ts`) — stored in `App.tsx` state and persisted in `FileHistory`.
...
- Set via right-click context menu ("Focus measure X") or the ranges drawer (bookmark-icon button, bottom-left); cleared via "Clear focus" in the context menu or "Whole piece" in the drawer. A focused range can be saved as a named **bookmark** ("Bookmark this range" in the context menu, or the + in the drawer's Bookmarks section) — see `src/hooks/use-bookmarks.ts`
- When non-null, `MidiPlayer.focusRange` **stops playback at the range end**: once `beat >= endBeat`, `startTick` stops (like a natural end-of-piece), resets the cursor and `resumeBeat` to the range start, and fires `onEnd` — so the next Play starts again from the range start rather than beat 0. `useWaitMode` also constrains wait points to that range.
- **Audio at the range start**: `MidiPlayer.startSchedule` builds its play queue via `notesSoundingAfter(notes, fromBeat)`, which keeps a note only when `startBeat + durationBeats > fromBeat + SEEK_END_TOLERANCE_BEATS`. The tolerance is the *audio* counterpart of the highlight measure-filter: the last chord before a focus range ends exactly on the barline, but its end beat float-overshoots the range-start beat by an ULP, so a strict `> fromBeat` test pulled it into the queue and — because its onset is now in the past — Web Audio sounded it immediately at the start of the range. The beat-based player has no measure grid to key on the way the highlight filter does, so a small beat tolerance (far below any real note, far above the ULP) is the right tool. This still keeps notes genuinely sustaining across a mid-note seek/resume. Regression: `tests/unit/play-queue.test.ts`.
- "Sounding now" highlights are constrained to the range **by rendered measure**, not by beat. The shared `soundingHighlights(notes, currentBeat, range, accent)` in `src/modes/note-colors.ts` filters each note through `measureInRange(note.measureIndex + 1, range)` before the sounding-span test, mirroring how wait mode's `rangeBounds` keys on `WaitPoint.measure`. This is load-bearing at the range's first barline: a note ending exactly there has `startBeat + durationBeats` that can float-overshoot the range-start beat by an ULP (the end is summed over a different division sequence than `computeMeasureStartBeats` accumulates), so the raw beat test alone highlights it as a phantom "first chord" left of the focus range. Keying on the integer measure sidesteps the float boundary entirely. **Both** listen mode and playalong's idle/count-in highlighting go through `soundingHighlights` — the `range` argument is required precisely so a future mode can't recompute sounding highlights without the filter and reintroduce the leak. (Playalong's *scoring* set, `selectionNotes`, was already range-safe: it filters by note onset via `graceMainBeat ?? startBeat`, which — unlike a note's end — never crosses the boundary.) Regression: `tests/unit/sounding-highlights.test.ts` against the `arabesque-no1.mxl` fixture, focus measure 6.
...
- **Bottom-left** — range selector (bookmark icon), Reset + Play/Pause + BPM buttons (row above in portrait, right of mode selector in landscape), Wait/Playalong/Listen mode selector; responsive via CSS `.bl-controls` / `.bl-transport` / `.bl-modes` classes
...
Playwright specs live in `tests/integration/` and run via `make integration-test`. To exercise the wait/playalong/listen pipelines deterministically, three browser APIs are mocked in `tests/integration/mocks/`:
...
- `bluetooth.ts` — installs a fake `navigator.bluetooth` whose `getDevices()` returns one device. The App's `useBluetooth` auto-reconnect flips status to `"connected"` on mount, and the unified `usePiano` controller surfaces that as connected, enabling the Wait and Playalong mode buttons. `window.__sendBleMidi(bytes)` dispatches a raw BLE-MIDI packet through the captured `characteristicvaluechanged` listener, exercising the full `parseBLEMIDI` → dispatch pipeline. Options let a spec turn the auto-reconnect off (`autoConnect: false`, so `getDevices()` returns nothing) or make the picker reject (`requestDeviceError`, simulating the user dismissing it); `window.__bleRequestDeviceCount()` reports how many times the picker was opened.
- `web-midi.ts` — installs a fake `navigator.requestMIDIAccess` plus a `navigator.permissions.query({name:"midi"})` result. The `permission` option is the interesting knob: `"granted"` exercises `useWebMidi`'s silent auto-reconnect, `"prompt"` asserts that nothing connects by itself. `window.__sendWebMidi(bytes)` fires a raw MIDI message at the port's `onmidimessage`, and `window.__webMidiRequestCount()` proves whether `requestMIDIAccess` was called at all (the permission guard must stop before it).
