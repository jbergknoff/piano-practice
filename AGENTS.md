...
- Highlights are constrained to the range **by rendered measure**, not by beat. `useListenMode`'s `soundingHighlights` filters each note through `measureInRange(note.measureIndex + 1, range)` before the sounding test, mirroring how wait mode's `rangeBounds` keys on `WaitPoint.measure`. This is load-bearing at the range's first barline: a note ending exactly there has `startBeat + durationBeats` that can float-overshoot the range-start beat by an ULP (the end is summed over a different division sequence than `computeMeasureStartBeats` accumulates), so the raw beat test alone highlights it as a phantom "first chord" left of the focus range. Keying on the integer measure sidesteps the float boundary entirely. (Regression: `tests/unit/listen-highlights.test.ts` against the `arabesque-no1.mxl` fixture, focus measure 6.)
