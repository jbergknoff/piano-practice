...
There is also a separate, self-contained **WYSIWYG MusicXML editor** under `editor/` (workspace member `@jbergknoff/editor`), intended to lift out to its own repo later. It owns a **duplicated** copy of the sheet-music renderer under `editor/src/sheet-music/` (extended in place with `ParsedNote.source` provenance and an additive pointer seam) and imports from that copy — **not** from `@jbergknoff/sheet-music-display`. It treats the live MusicXML `Document` as the source of truth and applies surgical DOM edits (`editor/src/dom-edit.ts`); coordinate inverses + note picking live in `editor/src/hit-test.ts`. Build with `make build-editor` / `make hot-reload-editor`; its unit tests run under the `editor` path in `make unit-test`. See `editor/PLAN.md` for the roadmap. The practice app does not depend on `editor/`.
